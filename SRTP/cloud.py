...
# 1、读入txt文本数据
text = open(r'result2.txt', "r").read()
...
# 3、初始化自定义背景图片
image = Image.open(r'C:\Users\Sylar\Desktop\SRTP\2.png')
graph = np.array(image)

# 4、产生词云图
# 有自定义背景图：生成词云图由自定义背景图像素大小决定
wc = WordCloud(font_path=r"C:\Windows\Fonts\msyhbd.ttc", background_color='white', max_font_size=70,
               mask=graph).generate(result)

# 5、绘制文字的颜色以背景图颜色为参考
image_color = ImageColorGenerator(graph)  # 从背景图片生成颜色值
wc.recolor(color_func=image_color)
wc.to_file(r"C:\Users\Sylar\Desktop\SRTP\1.png")  # 按照背景图大小保存绘制好的词云图，比下面程序显示更清晰
